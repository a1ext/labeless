...
// oddata.
  ...
  #define stdapi(type) extern                   type
  #define varapi(type) extern                   type
  #define oddata(type) extern     type
  #define pentry(type) extern     _export       type
#endif
...
/// Those ones aren't in OllyDbg2's EAT, so we comment their declarations
...
stdapi (ulong)   Readmemory(char *buf,ulong addr,ulong size,int mode);
%typemap(in) char *buf;
...
typedef void EMUFUNC(t_emu *,t_reg *);
...
stdapi (ulong)   Comparecommand(uchar *cmd,ulong cmdsize,ulong cmdip,
                   t_asmmod *model,int nmodel,int *INPUT,int *INPUT,t_disasm *da);
