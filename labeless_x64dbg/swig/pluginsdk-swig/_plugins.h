...
extern void _plugin_registercallback(int pluginHandle, CBTYPE cbType, CBPLUGIN cbPlugin);
extern bool _plugin_unregistercallback(int pluginHandle, CBTYPE cbType);
extern bool _plugin_registercommand(int pluginHandle, const char* command, CBPLUGINCOMMAND cbCommand, bool debugonly);
extern bool _plugin_unregistercommand(int pluginHandle, const char* command);
extern void _plugin_logprintf(const char* format, ...);
extern void _plugin_logputs(const char* text);
extern void _plugin_debugpause();
extern void _plugin_debugskipexceptions(bool skip);
extern int _plugin_menuadd(int hMenu, const char* title);
extern bool _plugin_menuaddentry(int hMenu, int hEntry, const char* title);
extern bool _plugin_menuaddseparator(int hMenu);
extern bool _plugin_menuclear(int hMenu);
extern void _plugin_menuseticon(int hMenu, const ICONDATA* icon);
extern void _plugin_menuentryseticon(int pluginHandle, int hEntry, const ICONDATA* icon);
extern void _plugin_startscript(CBPLUGINSCRIPT cbScript);
extern bool _plugin_waituntilpaused();
extern bool _plugin_registerexprfunction(int pluginHandle, const char* name, int argc, CBPLUGINEXPRFUNCTION cbFunction, void* userdata);
extern bool _plugin_unregisterexprfunction(int pluginHandle, const char* name);
