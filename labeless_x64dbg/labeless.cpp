...
extern "C" {
#ifdef _WIN64
	PyObject* PyInit__x64dbgapi64();
#else // _WIN64
	PyObject* PyInit__x64dbgapi();
#endif // _WIN64
};
...
static const char kX64DBGApiModuleName[] = {
#ifdef _WIN64
	"_x64dbgapi64"
#else // _WIN64
	"_x64dbgapi"
...
static PyObject* olly_log(PyObject*, PyObject* arg)
{
	if (PyUnicode_Check(arg))
		server_log(_T("%s"), util::to_xstr(PyUnicode_AsUTF8(arg)).c_str());
	Py_RETURN_NONE;
}
...
static PyObject* olly_get_ver(PyObject*, PyObject* arg)
{
	...
	PyObject* rv = PyUnicode_FromString(wver.c_str());
	return rv;
}
...
{
	if (g_pi.hProcess)
		return PyLong_FromSize_t(reinterpret_cast<size_t>(g_pi.hProcess));
	Py_RETURN_NONE;
}
...
{
	if (g_pi.dwProcessId)
		return PyLong_FromLong(g_pi.dwProcessId);
	Py_RETURN_NONE;
}
...
#endif // _WIN64
	PyDict_SetItemString(rv, "bitness", PyUnicode_FromString(kBitness));
...
bool Labeless::initPython()
{
...
#ifdef _WIN64
	PyImport_AppendInittab("_x64dbgapi64", &PyInit__x64dbgapi64);
#else // _WIN64
	...
	std::string apiModule = kX64DBGApiModuleName;
	err = PyRun_SimpleString(("import " + apiModule).c_str());
	if (err != 0) {
		logInitPythonFail("\"import " + apiModule + "\" failed.");
		return false;
	}
