...
class PEHelper(object):
    ...
    def _parse_nt_headers(self):
        ...
        if self._nt_headers.FileHeader.Machine != D.IMAGE_FILE_MACHINE_I386:
            print >> sys.stderr, 'Invalid PE header (Invalid machine type, supported only i386)'
            raise Exception('Invalid PE header (Invalid machine type, supported only i386)')
        self._opt_header = self._nt_headers.OptionalHeader
        if self._opt_header.Magic != D.OPTIONAL_HEADER_MAGIC_PE:
    ...
    def _parse_sections(self):
        ...
        offset = self._dos_header.e_lfanew + C.sizeof(D.IMAGE_NT_HEADERS)
        if numSections > 1000:
