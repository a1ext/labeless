...
typedef struct t_result {              // Result of expression's evaluation
  ...
  union {
    ...
    long double  f; } u;                 // Value as 80-bit float
  union {
...
%pybuffer_mutable_string(char *symb)
%pybuffer_mutable_string(char *comment)
extern int     Decodeaddress(ulong addr,ulong base,int addrmode,
               char *symb,int nsymb,char *comment);
%typemap(in) char *symb;
%typemap(in) char *comment;

extern int     Decoderelativeoffset(ulong addr,int addrmode,
               char *symb,int nsymb);
...
//extern int     IstextW(wchar_t w);
extern int     Stringtotext(char *data,int ndata,char *text,int ntext);

//data input protos
extern int     Getlong(char *title,ulong *data,int datasize,
               char letter,int mode);
extern int     Getlongxy(char *title,ulong *data,int datasize,
...
extern int     Getfloat10(char *title,long double *fdata,
               uchar *tag,char letter,int mode);
extern int     Getfloat10xy(char *title,long double *fdata,
               char *tag,char letter,int mode,int x,int y);
extern int     Getfloat(char *title,void *fdata,int size,
               char letter,int mode);
extern int     Getfloatxy(char *title,void *fdata,int size,
               char letter,int mode,int x,int y);
extern void    Getasmfindmodel(t_asmmodel model[NMODELS],
               char letter,int searchall);
extern void    Getasmfindmodelxy(t_asmmodel model[NMODELS],
               char letter,int searchall,int x,int y);
extern int     Gettext(char *title,char *text,
               char letter,int type,int fontindex);
extern int     Gettextxy(char *title,char *text,char letter,
               int type,int fontindex,int x,int y);
extern int     Gethexstring(char *title,t_hexstr *hs,
               int mode,int fontindex,char letter);
extern int     Gethexstringxy(char *title,t_hexstr *hs,int mode,
...
extern int     Browsefilename(char *title,char *name,char *defext,
               int getarguments);
...
%pybuffer_mutable_string(char *buf)
extern ulong   Readmemory(char *buf,ulong addr,ulong size,int mode);
%typemap(in) char *buf;

extern ulong   Writememory(char *buf,ulong addr,ulong size,int mode);

%pybuffer_mutable_string(char *cmd)
...
extern int     Registerpluginclass(char *classname,char *iconname,
									   HINSTANCE dllinst,WNDPROC classproc);
extern void    Unregisterpluginclass(char *classname);
extern int     Pluginwriteinttoini(HINSTANCE dllinst,char *key,int value);
extern int     Pluginwritestringtoini(HINSTANCE dllinst,char *key,char *s);
extern int     Pluginreadintfromini(HINSTANCE dllinst,char *key,int def);
extern int     Pluginreadstringfromini(HINSTANCE dllinst,char *key,char *s,char *def);
extern int     Pluginsaverecord(ulong tag,ulong size,void *data);
extern int     Plugingetvalue(int type);
...
%pointer_cast(void*, t_ref*, void_to_t_ref);
%pointer_cast(void*, t_patch*, void_to_t_patch);


/*%inline %{
t_table* PlugingetvalueTable(int type)
{
    return (t_table*)Plugingetvalue(type);
}

t_memory* MemoryFromVoid(void* p)
{
    return (t_memory*)p;
}

t_module* ModuleFromVoid(void* p)
{
	return (t_module*)p;
}
...
t_table* PlugingetvalueTable(int type)
{
    void* rv = (void*)Plugingetvalue(type);
    if (!rv)
    {
        $result = NULL;
    }
    ...
    {
        t_table* table = (t_table*)rv;
        switch (type)
        {
        case VAL_MODULES:
        case VAL_MEMORY:
        case VAL_THREADS:
        case VAL_BREAKPOINTS:
        case VAL_REFERENCES:
        case VAL_SOURCELIST:
        case VAL_WATCHES:
        case VAL_PATCHES:
            $result = SWIG_NewPointerObj(SWIG_as_voidptr(temp$argnum), SWIGTYPE_p_t_table, 0 | 0 );
            break;
        default:
            $result = NULL;
